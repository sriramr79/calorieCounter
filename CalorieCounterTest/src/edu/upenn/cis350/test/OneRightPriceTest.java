...
public class OneRightPriceTest extends
	...
	public OneRightPriceTest(Class<OneRightPriceActivity> activityClass) {
	...
	protected void setUp() throws Exception {
		super.setUp();
		 ...
		 mView = (OneRightPriceView)mActivity.findViewById(edu.upenn.cis350.R.id.rankingView);
	}
	...
	/**
	 * Tests to see whether the setup procedures for the OneRightPrice class are run properly
	 */
...
public void testOnTouchEvent() {
		...
		mActivity.runOnUiThread(
	    	      	new Runnable() {
	    	      	public void run() {
	    	        	mView.fakeTouchEvent(MotionEvent.ACTION_DOWN, mView.calorieNumberSquare.getCenterX(), mView.calorieNumberSquare.getCenterY());
	    ...
	    mActivity.runOnUiThread(
    	      	new Runnable() {
    	      	public void run() {
    	        	mView.fakeTouchEvent(MotionEvent.ACTION_MOVE, mView.calorieNumberSquare.getCenterX()+50, mView.calorieNumberSquare.getCenterY()+50);
    	        } // end of run() method definition
    	      } // end of anonymous Runnable object instantiation
    	    ); // end of invocation of runOnUiThread
	    ...
	    assertNotNull(mView.touchedSquare);
	    assertEquals(mView.food0Square.getCenterX() + 10, mView.calorieNumberSquare.getCenterX());
	    assertEquals(mView.food0Square.getCenterY() + 10, mView.calorieNumberSquare.getCenterY());
	    
	    // Make sure that the square does snap when dropped
	    mActivity.runOnUiThread(
    	      	new Runnable() {
    	      	public void run() {
    	        	mView.fakeTouchEvent(MotionEvent.ACTION_UP, mView.calorieNumberSquare.getCenterX(), mView.calorieNumberSquare.getCenterY());
    	        } // end of run() method definition
    	      } // end of anonymous Runnable object instantiation
    	    ); // end of invocation of runOnUiThread

	    assertNull(mView.touchedSquare);
	    assertEquals(mView.food0Square.getCenterX(), mView.calorieNumberSquare.getCenterX());
	    assertEquals(mView.food0Square.getCenterY(), mView.calorieNumberSquare.getCenterY());
